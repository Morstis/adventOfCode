let input = `ZNNvFWHqLNPZHHqPTHHnTGBhrrpjvmwfMmpfpjBjwpmw
...
let newdata = [];
while (s.length) {
  newdata.push(s.splice(0, 3).join(",").split(","));
}

newdata = newdata.sort((a, b) => b.length - a.length);
console.log(newdata);
...
const res = newdata
  .map((x) => {
    ...
    for (let i = 0; i < x[0].length; i++) {
      if (x[1].includes(x[0][i]) && x[2].includes(x[0][i])) items.add(x[0][i]);
    }
